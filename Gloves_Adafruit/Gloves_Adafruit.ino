...
#define PREV_SONG_CMD "MEDIANEXT"
...
void getCommand(char buffer[], uint8_t maxSize)
{
  ...
  while(digitalRead(VOL_UP) == LOW &&
        digitalRead(VOL_DOWN) == LOW &&
        digitalRead(PLAY_PAUSE) == LOW &&
        digitalRead(PREV_SONG) == LOW) {
            //Serial.println("waiting...");
            delay(1);
            }

  if(digitalRead(VOL_UP) == HIGH) {
    strcpy(cmd, VOL_UP_CMD);
  } else if(digitalRead(VOL_DOWN) == HIGH) {
    strcpy(cmd, VOL_DOWN_CMD);
  } else if(digitalRead(PLAY_PAUSE) == HIGH) {
    strcpy(cmd, PLAY_PAUSE_CMD);
  } else if(digitalRead(PREV_SONG) == HIGH) {
    strcpy(cmd, PREV_SONG_CMD);
  }
